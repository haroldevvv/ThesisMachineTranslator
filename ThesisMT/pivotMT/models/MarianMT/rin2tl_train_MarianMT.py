...
file_path = "/ThesisMT/Datasets/real_parallel/generated_synthetic_data/pivot_translation/MarianMT/tl_rin_3.5k_MarianMT.csv"   
...
def prepare_dataset(file_path, tokenizer):
    ...
    assert "tl" in df.columns and "rin" in df.columns, \
        "CSV must have 'tl' (Tagalog) and 'rin' (Rinconada) columns!"

    df = df.dropna(subset=["tl", "rin"])
    df["tl"] = df["tl"].astype(str)
    df["rin"] = df["rin"].astype(str)

    print(f"Loaded {len(df)} parallel sentence pairs (Rinconada → Tagalog)")
...
def main():
    ...
    experiment_name = "rin_tl_3.5k_25epochs"
